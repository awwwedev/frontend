<template>
  <div class="view-object">
    ...
    >
      ...
      <form action="" slot="content" class="form" ref="form">
        <div class="row form__row form__row_no-margin-sm">
          <div class="form__group col-6 col-12-sm"
               :class="{ 'invalid': $v.formModels.fio.$invalid && $v.formModels.fio.$dirty }">
            <label for="fio" class="form__label">ФИО</label>
            <input type="text" id="fio" class="form__control" v-model="formModels.fio"
                   @input="$v.formModels.fio.$touch()">
            <span class="form__invalid-message">
                        Это поле обязательно для заполнения
                      </span>
          </div>
          <div class="form__group col-6 col-12-sm"
               :class="{ 'invalid': $v.formModels.phone.$invalid && $v.formModels.phone.$dirty }">
            <label for="phone" class="form__label">Номер телефона</label>
            <input type="tel" id="phone" class="form__control" v-mask="'+7 (###) ###-##-##'" v-model="formModels.phone"
                   @input="$v.formModels.phone.$touch()">
            <span class="form__invalid-message">
        ...
        <div class="form__group" :class="{ 'invalid': $v.formModels.email.$invalid && $v.formModels.email.$dirty }">
          ...
          <input type="email" id="email" class="form__control" v-model="formModels.email"
                 @input="$v.formModels.email.$touch()">
          <span class="form__invalid-message">
        ...
        <div class="form__group">
          ...
          <textarea id="message" class="form__control" rows="4" v-model="formModels.message"
                    @input="$v.formModels.message.$touch()"></textarea>
        </div>
      </form>
      <button class="btn btn_primary btn_sm" slot="btn-ok" @click="onSubmit" :disabled="$v.$invalid">Отправить</button>
    </Modal>
...
import Notification from "@/js/components/ui/Notification.vue";
...
@Component({
  ...
  components: {Notification, RealtyView, Modal, Object, Slider},
...
export default class ViewObject extends Mixins<Validation>(validationMixin) {
  ...
  @Ref('notification') refNotification!: Notification
